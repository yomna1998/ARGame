...
public class WritingHandler : MonoBehaviour
{
	...
	public GameObject winDialog;//win dialog
	public Image winDialogLetterSprite;//letter sprite renderer
	...
	void Update()
	{
		...
		if (hand != null)
		{
			...
			//drag the hand on screen - start

			Vector3 input = Input.mousePosition;
			input.x -= ((hand.rect.width * HandCanvus.scaleFactor) / 2); // The important part!
			...
			Vector3 output;
			RectTransformUtility.ScreenPointToWorldPointInRectangle(hand, input, GetComponent<Camera>(), out output);
			hand.position = output;

			//drag the hand on screen - end
	...
	private void EndTouchLetterHandle()
	{
		...
		{//if the current letter done or wirrten successfully
			...
			hand.GetComponent<Image>().enabled = false;//hide the hand
		}
	}
    ...
    public static bool onSatate = true;
